...
class CapabilityRegistry:
    ...
    async def _discover_caps_by_type(self, svr_name: str,
        ...
        try:
            ...
            logger.debug(
                f"[{svr_name}] Requesting {cap_type} list (timeout {CAP_FETCH_TIMEOUT}s)...")
            ...
            if hasattr(list_result, cap_type) and isinstance(
                ...
                logger.info(
                    f"[{svr_name}] {list_method_name}() returned None, treating as no {cap_type}."
                )
                orig_caps = []
            else:
                logger.warning(
                    f"[{svr_name}] {list_method_name}() returned unknown type "
                    f"{type(list_result)}; unable to parse {cap_type} list. Raw value: {list_result!r}"
                )
                orig_caps = []
            ...
            for cap_item_raw in orig_caps:
                ...
                if not isinstance(cap_item_raw, mcp_cls):
                    logger.warning(
                        f"[{svr_name}] Found non-{mcp_cls.__name__} object, skipped: {cap_item_raw!r}"
                    )
                    continue
                ...
                if not cap_item.name:
                    logger.warning(
                        f"[{svr_name}] Found unnamed {cap_type[:-1]}, skipped: {cap_item!r}"
                    )
                    continue
                ...
                if exp_cap_name in self._route_map:
                    ...
                    if exist_svr_name != svr_name:
                        logger.warning(
                            f"Conflict: {cap_type[:-1]} '{exp_cap_name}' is already registered "
                            f"by server '{exist_svr_name}'. The duplicate from server "
                            f"'{svr_name}' will be ignored.")

                        continue
                    else:
                        logger.warning(
                            f"[{svr_name}] Duplicate {cap_type[:-1]} provided multiple times: "
                            f"'{exp_cap_name}'. Only the first instance is registered."
                        )
                        continue
            ...
            if registered_count > 0:
                ...
                logger.info(f"[{svr_name}] No new {cap_type} discovered or registered.")

        except asyncio.TimeoutError:
            logger.error(
                f"[{svr_name}] {list_method_name}() timed out (>{CAP_FETCH_TIMEOUT}s)."
            )
            ...
            logger.exception(f"[{svr_name}] Unknown error while discovering {cap_type}.")
    ...
    async def discover_and_register(self, sessions: Dict[str, ClientSession]):
        ...
        logger.info(f"Starting capability discovery/registration from {len(sessions)} active sessions...")
        ...
        for svr_name, session in sessions.items():
            if not session:
                logger.warning(f"Skipping server '{svr_name}' because it has no valid session.")
                continue
        ...
        logger.info(f"Aggregated discovery: {len(self._tools)} tools, "
