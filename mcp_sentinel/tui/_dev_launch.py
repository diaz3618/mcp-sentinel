...
latest source code we purge all ``mcp_sentinel`` sub-modules before
re-importing ``SentinelApp``.
"""
...
# Purge cached mcp_sentinel modules so the MCP server always uses fresh code.
_stale = [k for k in sys.modules if k == "mcp_sentinel" or k.startswith("mcp_sentinel.")]
for _k in _stale:
    del sys.modules[_k]

import mcp_sentinel.tui.app  # noqa: E402
